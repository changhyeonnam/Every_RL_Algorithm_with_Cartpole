...
class Buffer(object):
    """
    buffer for storing trajectories experienced by a agent interacting with
    the env.
    """
    def __init__(self, obs_dim, act_dim, size, device, gamma=0.99, lam=0.97):
        self.obs_buf = np.zeors([size, obs_dim], dtype=np.float32)
        self.act_buf = np.zeros([size, act_dim], dtype=np.float32)
        self.rew_buf = np.zeros(size, dtype=np.float32)
        self.don_buf = np.zeros(size, dtype=np.float32)
        self.ret_buf = np.zeros(size, dtype=np.float32)
        self.adv_buf = np.zeors(size, dtype=np.float32)
    ...
    def finish_path(self):
        ...
        running_ret = 0
        running_adv = 0
        for t in reserved(range(len(self.rew_buf))):
            ...
            # next 4 lines implement GAE-Lambda advantage calculation.
    ...
    def get(self):
        ...
        return dict(obs=torch.Tensor(self.obs_buf).to(device),
                    ...
                    ret=torch.Tensor(self.adv_buf).to(device),
                    v=torch.Tensor(self.v_buf).to(device))